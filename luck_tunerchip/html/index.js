...
function menuToggle(bool,send=false) {
	...
	if(send){
		$.post('http://luck_tunerchip/togglemenu', JSON.stringify({state:false}));
	}
}
...
$(function(){
	...
	$("#boost").slider({min:0.05,value:0.15,step:0.01,max:0.25,change:sliderUpdated});
	$("#fuelmix").slider({min:1,value:1.5,step:0.01,max:1.8,change:sliderUpdated});
	$("#gearchange").slider({min:1,value:15,max:40,step:10,change:sliderUpdated});
	...
	$("#savebtn").click(function(){
		initiateTyepwriter();
		$.post('http://luck_tunerchip/save', JSON.stringify(getSliderValues()));
	});
